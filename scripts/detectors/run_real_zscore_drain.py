...
def trigger_fault():
    ...
    cmd = f'ssh -i ~/.ssh/id_ed25519 -o StrictHostKeyChecking=no jisheng3@sp26-cs525-0605.cs.illinois.edu "timeout 30 python3 -c \\"import multiprocessing; f=lambda: sum(1 for _ in iter(int, 1)); [multiprocessing.Process(target=f).start() for _ in range(32)]\\""'
